...
git push origin develop/v2.8.0  # 应该被拒绝！
```
...
# 2. 修改解析器文件
# 编辑 crates/parser/src/parser.rs
# 实现 parse_limit 函数

# 3. 添加测试
# 编辑 crates/parser/tests/parser_test.rs
# 添加 LIMIT 相关测试用例
...
git add crates/parser/tests/parser_test.rs
...
1. 打开 GitHub 仓库
2. 点击 "Compare & pull request"
3. 选择 `feature/ai-parser-limit` → `develop/v2.8.0`
4. 填写PR描述
...
# 模拟学生A的修改
echo "let limit = 10;" >> crates/parser/src/parser.rs
git commit -m "test: simulate student A change"
...
# 模拟学生B的修改
echo "let limit = 100;" >> crates/parser/src/parser.rs
git commit -m "test: simulate student B change"
...
2. **编辑冲突文件**：
   - 打开 `crates/parser/src/parser.rs`
   - 查看冲突标记
   - 选择正确的版本或合并两个版本
   - 删除冲突标记
...
# 实现 ORDER BY 语法支持
# 提交代码
git push origin feature/ai-parse-order-by
